  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (d.error) {
        ...
        throw new Error(`AI: ${d.error}`);
      }
      const text = d.content || d.text || (typeof d === "string" ? d : "");
      if (!text) throw new Error("AI returned empty response");
      return text;
    } catch(e) {
      if (attempt < retries && !e.message?.startsWith("AI:")) {
        await new Promise(ok => setTimeout(ok, (attempt+1)*2000));
        continue;
      }
      throw e;
...
function V9() {
  ...
  // Webhook management
  const [whConfigs, setWhConfigs] = useState([]);
  const [whLogs, setWhLogs] = useState([]);
  const [whEditing, setWhEditing] = useState(null);
  const [whForm, setWhForm] = useState({name:"",url:"",type:"custom",events:[],headers:{},method:"POST",payloadTemplate:"",secret:"",retryCount:2,timeoutMs:10000});
  const [whTesting, setWhTesting] = useState(null);
  const [whSupportedEvents, setWhSupportedEvents] = useState([]);
  ...
  useEffect(()=>{(async()=>{
    ...
    try{const whR=await api._f("/api/admin/webhook-config");if(whR?.webhooks){setWhConfigs(whR.webhooks);setWhSupportedEvents(whR.supportedEvents||[]);}}catch(e){}
    setLoading(false);
  })();},[]);
  ...
  // ── Webhook helpers ──
  const whLoad = async()=>{try{const r=await api._f("/api/admin/webhook-config");if(r?.webhooks){setWhConfigs(r.webhooks);setWhSupportedEvents(r.supportedEvents||[]);}}catch(e){log("❌ Webhook load failed","error");}};
  const whLoadLogs = async(id)=>{try{const r=await api._f(`/api/admin/webhook-config/logs${id?`?webhookId=${id}`:""}&limit=50`);if(r?.logs)setWhLogs(r.logs);}catch(e){}};
  const whSave = async()=>{
    const body=JSON.stringify(whForm);
    if(whEditing){
      await api._f(`/api/admin/webhook-config/${whEditing}`,{method:"PUT",body});
      log("✅ Webhook updated","success");
    }else{
      await api._f("/api/admin/webhook-config",{method:"POST",body});
      log("✅ Webhook created","success");
    }
    setWhEditing(null);setWhForm({name:"",url:"",type:"custom",events:[],headers:{},method:"POST",payloadTemplate:"",secret:"",retryCount:2,timeoutMs:10000});
    whLoad();
  };
  const whDelete = async(id)=>{await api._f(`/api/admin/webhook-config/${id}`,{method:"DELETE"});log("🗑️ Webhook deleted");whLoad();};
  const whTest = async(id)=>{setWhTesting(id);try{const r=await api._f(`/api/admin/webhook-config/${id}/test`,{method:"POST"});log(r?.success?"✅ Test successful":"❌ Test failed: "+(r?.error||"Unknown"),r?.success?"success":"error");}catch(e){log("❌ Test error: "+e.message,"error");}setWhTesting(null);};
  const whToggle = async(id,active)=>{await api._f(`/api/admin/webhook-config/${id}`,{method:"PUT",body:JSON.stringify({active:!active})});whLoad();};
  const whEdit = (w)=>{setWhEditing(w._id||w.id);setWhForm({name:w.name,url:w.url,type:w.type||"custom",events:w.events||[],headers:w.headers||{},method:w.method||"POST",payloadTemplate:w.payloadTemplate||"",secret:w.secret||"",retryCount:w.retryCount||2,timeoutMs:w.timeoutMs||10000});};
  const whToggleEvent = (ev)=>setWhForm(f=>({...f,events:f.events.includes(ev)?f.events.filter(e=>e!==ev):[...f.events,ev]}));
  const whDispatch = async(event)=>{try{const r=await api._f("/api/admin/webhook-config/dispatch",{method:"POST",body:JSON.stringify({event,data:{test:true,message:`Manual dispatch: ${event}`,timestamp:new Date().toISOString()}})});log(`📡 Dispatched ${event} → ${r?.results?.length||0} webhooks`,"success");}catch(e){log("❌ Dispatch error","error");}};
      ...
      <main style={{flex:1,marginLeft:0,minHeight:"100vh",background:"#050505"}} className="main-content">
        ...
        <div style={{padding:"20px",maxWidth:1400,margin:"0 auto"}}>
          ...
          {view === "webhooks" && (
            <div className="anim-fade" style={{display:"flex",flexDirection:"column",gap:16}}>
              <Panel>
                <Hdr icon={<Link size={16}/>} title="Webhook Integrations" sub="Connect any service — AI agents, Discord, Slack, n8n, Make, Zapier, Clawdbot, or custom"/>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
                  <Btn onClick={()=>{setWhEditing(null);setWhForm({name:"",url:"",type:"custom",events:[],headers:{},method:"POST",payloadTemplate:"",secret:"",retryCount:2,timeoutMs:10000});}}><Zap size={12}/> New Webhook</Btn>
                  <Btn variant="ghost" onClick={()=>whLoadLogs()}><Activity size={12}/> View Logs</Btn>
                  <Btn variant="ghost" onClick={whLoad}><RefreshCw size={12}/> Refresh</Btn>
                </div>

                {/* Webhook Form */}
                {(whEditing!==undefined && whEditing!==null) || whForm.name || whForm.url ? (
                  <div style={{background:"rgba(99,102,241,.05)",border:"1px solid rgba(99,102,241,.15)",borderRadius:10,padding:16,marginBottom:16}}>
                    <div style={{fontSize:13,fontWeight:600,color:"#a5b4fc",marginBottom:12}}>{whEditing?"Edit Webhook":"New Webhook"}</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      <div><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Name</label><input value={whForm.name} onChange={e=>setWhForm(f=>({...f,name:e.target.value}))} placeholder="My Integration"/></div>
                      <div><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Type</label><select value={whForm.type} onChange={e=>setWhForm(f=>({...f,type:e.target.value}))}><option value="custom">Custom</option><option value="discord">Discord</option><option value="slack">Slack</option><option value="clawdbot">Clawdbot</option><option value="n8n">n8n</option><option value="make">Make</option><option value="zapier">Zapier</option><option value="ai_agent">AI Agent</option></select></div>
                      <div style={{gridColumn:"1/-1"}}><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Webhook URL</label><input value={whForm.url} onChange={e=>setWhForm(f=>({...f,url:e.target.value}))} placeholder="https://hooks.example.com/..."/></div>
                      <div><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Method</label><select value={whForm.method} onChange={e=>setWhForm(f=>({...f,method:e.target.value}))}><option value="POST">POST</option><option value="PUT">PUT</option><option value="PATCH">PATCH</option></select></div>
                      <div><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Secret (HMAC signing)</label><input value={whForm.secret} onChange={e=>setWhForm(f=>({...f,secret:e.target.value}))} placeholder="Optional signing secret"/></div>
                      <div style={{gridColumn:"1/-1"}}><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:3}}>Payload Template (optional — use {"{{event}}"}, {"{{data}}"}, {"{{timestamp}}"})</label><textarea value={whForm.payloadTemplate} onChange={e=>setWhForm(f=>({...f,payloadTemplate:e.target.value}))} placeholder='{"action":"{{event}}","payload":{{data}}}' rows={2}/></div>
                    </div>
                    <div style={{marginTop:12}}><label style={{fontSize:10,color:"#71717a",display:"block",marginBottom:6}}>Subscribe to Events</label>
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {(whSupportedEvents.length>0?whSupportedEvents:["order.created","product.discovered","product.analyzed","product.listed","product.killed","pipeline.completed","alert.error","webhook.test"]).map(ev=>(
                          <button key={ev} onClick={()=>whToggleEvent(ev)} style={{padding:"3px 10px",borderRadius:6,fontSize:10,fontWeight:500,cursor:"pointer",border:"1px solid",fontFamily:"inherit",transition:"all .15s",background:whForm.events.includes(ev)?"rgba(99,102,241,.2)":"transparent",color:whForm.events.includes(ev)?"#a5b4fc":"#52525b",borderColor:whForm.events.includes(ev)?"rgba(99,102,241,.3)":"#27272a"}}>{ev}</button>
                        ))}
                      </div>
                    </div>
                    <div style={{display:"flex",gap:8,marginTop:14}}>
                      <Btn onClick={whSave} disabled={!whForm.name||!whForm.url}><Check size={12}/> {whEditing?"Update":"Create"}</Btn>
                      <Btn variant="ghost" onClick={()=>{setWhEditing(null);setWhForm({name:"",url:"",type:"custom",events:[],headers:{},method:"POST",payloadTemplate:"",secret:"",retryCount:2,timeoutMs:10000});}}><X size={12}/> Cancel</Btn>
                    </div>
                  </div>
                ) : null}
                ...
                {/* Webhook List */}
                {whConfigs.length===0?(
                  <div style={{textAlign:"center",padding:"40px 20px"}}><Link size={32} style={{color:"#27272a",margin:"0 auto 10px"}}/><p style={{color:"#52525b",fontSize:12}}>No webhooks configured</p><p style={{color:"#3f3f46",fontSize:11,marginTop:4}}>Create one to start integrating with external services</p></div>
                ):(
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {whConfigs.map(w=>{
                      const wid=w._id||w.id;
                      const typeColors={discord:"#5865F2",slack:"#E01E5A",clawdbot:"#22c55e",n8n:"#ff6d5a",make:"#6d28d9",zapier:"#ff4a00",ai_agent:"#06b6d4",custom:"#71717a"};
                      return(
                        <div key={wid} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:w.active?"rgba(255,255,255,.02)":"rgba(255,255,255,.01)",border:`1px solid ${w.active?"#1a1a1a":"#111"}`,borderRadius:10,opacity:w.active?1:.5}}>
                          <div style={{width:8,height:8,borderRadius:"50%",background:w.active?"#22c55e":"#ef4444",flexShrink:0}}/>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              <span style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>{w.name}</span>
                              <span style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:`${typeColors[w.type]||"#71717a"}22`,color:typeColors[w.type]||"#71717a",fontWeight:600,textTransform:"uppercase"}}>{w.type}</span>
                            </div>
                            <div style={{fontSize:11,color:"#52525b",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{w.url}</div>
                            <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
                              {(w.events||[]).slice(0,5).map(ev=><span key={ev} style={{fontSize:8,padding:"1px 5px",borderRadius:3,background:"rgba(99,102,241,.1)",color:"#818cf8"}}>{ev}</span>)}
                              {(w.events||[]).length>5&&<span style={{fontSize:8,color:"#52525b"}}>+{w.events.length-5}</span>}
                            </div>
                          </div>
                          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,flexShrink:0}}>
                            <div style={{fontSize:10,color:"#52525b"}}>{w.totalSent||0} sent · {w.totalFailed||0} failed</div>
                            {w.lastTriggered&&<div style={{fontSize:9,color:"#3f3f46"}}>Last: {new Date(w.lastTriggered).toLocaleString()}</div>}
                          </div>
                          <div style={{display:"flex",gap:4,flexShrink:0}}>
                            <Btn size="sm" variant="ghost" onClick={()=>whTest(wid)} loading={whTesting===wid}><Zap size={10}/></Btn>
                            <Btn size="sm" variant="ghost" onClick={()=>whEdit(w)}><Edit size={10}/></Btn>
                            <Btn size="sm" variant="ghost" onClick={()=>whToggle(wid,w.active)}>{w.active?<Pause size={10}/>:<Play size={10}/>}</Btn>
                            <Btn size="sm" variant="ghost" onClick={()=>whDelete(wid)} style={{color:"#f87171"}}><Trash2 size={10}/></Btn>
                          </div>
                        </div>
                      );
                    })}
              ...
              <Panel>
                <Hdr icon={<Zap size={16}/>} title="Quick Connect" sub="One-click setup for popular services"/>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
                  {[
                    {name:"Discord",type:"discord",icon:"🟣",desc:"Send notifications to Discord channel",placeholder:"https://discord.com/api/webhooks/..."},
                    {name:"Slack",type:"slack",icon:"💬",desc:"Post updates to Slack channel",placeholder:"https://hooks.slack.com/services/..."},
                    {name:"Clawdbot",type:"clawdbot",icon:"🤖",desc:"Trigger Clawdbot AI agent actions",placeholder:"https://your-agent.preview.emergentagent.com/api/webhook"},
                    {name:"n8n",type:"n8n",icon:"🔧",desc:"Trigger n8n automation workflows",placeholder:"https://your-n8n.app.n8n.cloud/webhook/..."},
                    {name:"Make",type:"make",icon:"⚡",desc:"Start Make (Integromat) scenarios",placeholder:"https://hook.us1.make.com/..."},
                    {name:"Zapier",type:"zapier",icon:"🔌",desc:"Trigger Zapier automations",placeholder:"https://hooks.zapier.com/hooks/catch/..."},
                    {name:"AI Agent",type:"ai_agent",icon:"🧠",desc:"Send events to custom AI agent",placeholder:"https://your-agent.com/webhook"},
                    {name:"Custom API",type:"custom",icon:"🌐",desc:"Any HTTP endpoint",placeholder:"https://api.example.com/webhook"},
                  ].map(t=>(
                    <button key={t.type} onClick={()=>{setWhEditing(null);setWhForm({...whForm,name:t.name+" Webhook",type:t.type,url:"",events:["order.created","product.discovered"],method:"POST",payloadTemplate:"",secret:""});}} style={{padding:14,borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid #1a1a1a",cursor:"pointer",textAlign:"left",fontFamily:"inherit",transition:"all .2s"}}>
              ...
              {whLogs.length>0&&(
                <Panel>
                  <Hdr icon={<Activity size={16}/>} title={`Recent Logs (${whLogs.length})`}/>
                  <div style={{maxHeight:400,overflowY:"auto"}}>
                    {whLogs.map((l,i)=>(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #111",fontSize:11}}>
                        <span style={{width:6,height:6,borderRadius:"50%",background:l.success?"#22c55e":"#ef4444",flexShrink:0}}/>
                        <span style={{color:"#818cf8",minWidth:80}}>{l.event}</span>
                        <span style={{color:"#71717a",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.webhookName}</span>
                        <span style={{color:l.success?"#4ade80":"#f87171",fontWeight:600}}>{l.status||"ERR"}</span>
                        <span style={{color:"#3f3f46"}}>{l.responseTime}ms</span>
                        <span style={{color:"#27272a",fontSize:9}}>{new Date(l.createdAt).toLocaleTimeString()}</span>
                      </div>
                    ))}
                  </div>
                </Panel>
              ...
              {/* Event Dispatch */}
              <Panel>
                <Hdr icon={<Send size={16}/>} title="Manual Event Dispatch" sub="Trigger events manually to test integrations"/>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {["order.created","product.discovered","product.analyzed","product.listed","pipeline.completed","alert.error","webhook.test"].map(ev=>(
                    <Btn key={ev} size="sm" variant="ghost" onClick={()=>whDispatch(ev)}><Send size={10}/> {ev}</Btn>
                  ))}
                </div>
              </Panel>
            </div>
